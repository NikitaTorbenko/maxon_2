...
import { getOrders } from "../api/stataService";
...
<template>
  <layout is-effect-background class="h-screen">
    <div class="flex items-center mt-5 cursor-pointer">
      ...
      <h2
        class="font-bold text-xl leading-[29px] text-[#FFC01E] w-[74px] h-[30px]"
      >
        ЗАКАЗЫ
      </h2>
    </div>
    <div>
      <p
        class="font-normal text-sm leading-4 text-[#FFFFFF] text-center mt-28"
        v-if="orders.length === 0"
      >
        История заказов пуста.<br />Перейдите в каталог и начните покупки
      </p>
    </div>
    ...
    <div class="fixed left-0 bottom-4 w-full flex items-center justify-center">
      <Button
        class="bg-[#FFC01E] w-[90%] text-white h-[51px]"
        @click="router.push('/')"
        >В каталог</Button
      >
    </div>
    <div>
      <h1>Диаграмма продаж</h1>
      <ChartComponent />
    </div>
  </layout>
</template>
